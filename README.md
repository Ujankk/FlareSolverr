...
| download            | Optional, default false. After the challenge is solved, download asset URL(s) with the browser `fetch()` API (same TLS fingerprint as the page). Results are in `solution.download`. If a file cannot be downloaded or the body is HTML (block page), `data` is the string `failed`. |
| downloadUrls        | Optional. List of absolute URLs to download when `download` is true. If omitted, `url` is downloaded. Example: `"downloadUrls": ["https://example.com/a.png", "https://example.com/b.css"]`.                                                                                    |
